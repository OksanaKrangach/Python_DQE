...
import json
from datetime import datetime
from strings_func_4 import process_text
...
class FileManager:
    ...
    def read_file(p_file_path: str) -> str:
        ...
        file_format = os.path.splitext(p_file_path)[1]

        # if file_format == '.json':
        #     file_data = self.__read_json_file(absolute_file_path)
        # elif file_format in ('.txt', '.html'):
        #     file_data = self.__read_text_file(absolute_file_path)
        # else:
        #     raise ValueError(f"File format '{file_format}' is currently not supported for reading")
...
def main():
    ...
    while not exit_requested:
        ...
        if input_type == "0":
            ...
            input_file_path = os.path.join(os.getcwd(), txt_input_file_name) if input_type == '2' else \
                os.path.join(os.getcwd(), json_input_file_name)
            print(f"\nDefault file path is {input_file_path}")
            local_exit_requested = False
            while not local_exit_requested and not exit_requested:
                choice = \
